...
In order to compare our observations to predictions from $\lcdm$, we adopt two different DM simulations. One of these is the MICE-GC $N$-body simulation \cite[]{fosalba2015b}, which contains $\sim 7$$\E{10}$ DM particles in a $(3072 \hsMpc)^3$ comoving volume. From this simulation the MICE collaboration constructed a $\sim5000\deg^2$ lightcone, with a maximum redshift of $z=1.4$. In this work, we u The Friends of Friends (FOF) DM halos in this lightcone were populated with galaxies using a hybrid Halo Occupation Distribution (HOD) and Halo Abundance Matching (HAM) prescription \cite[]{carretero2015,crocce2015}. Halos are considered 'resolved' down to 20 particles, corresponding to a halos with a mass of $6\E{11} \hsmsun$ which host galaxies with an absolute magnitude $<-18.9$ (*should we apply a cut here?*). Every galaxy has a set of sky coordinates, a redshift and comoving distance associated with it. In version 2.0 of the MICE catalogue, which we use in this work, each galaxy is also assigned a stellar mass (*how was this determined*) needed in order to compute the RAR (see Sect. \ref{sec:conversion}). 
