...
\author[M. M. Brouwer]{Margot M. Brouwer$^{1,2}$\thanks{E-mail:brouwer@astro.rug.nl},
  K. A. Oman et al.$^{1}$
  %Group 1:
  %Group 2:
  %Group 3:
...
To remove any stellar contamination we apply three independent cuts, based on galaxy morphology, the 9-band photometry and the SExtractor star/galaxy classifier respectively.\footnote{Our star-galaxy separation corresponds to applying the following flags: SG2DPHOT=0, SG\_FLAG=1, CLASS\_STAR<0.5.} We remove galaxies that are affected by readout and diffraction spikes, by saturation cores, by bad pixels, and by primary, secondary or tertiary haloes of bright stars.\footnote{This masking corresponds to applying all MASK values (1, 2, 4, 8, 16, 32 and 64) described in App. A.1.1 of \cite{kuijken2019}, through applying the flag: IMAFLAGS\_ISO=0.} In addition, we apply the recommended `9-band no AW-$r$-band mask', which is also used to create the KiDS-DR4 shear catalogues.\footnote{This masking corresponds 9-band MASK bit values 2 - 11, 13 and 14, described in App. A.2 of \cite{kuijken2019}.} For more information on these masks, we refer the reader to App. A of \cite{kuijken2019}. Our final sample GL-KiDS lenses consists of ... galaxies with a mean redshift of ... .
...
When comparing $M\un{*,ANN}$ with the GAMA stellar masses $M\un{*,G}$ of matched galaxies, we find that its distribution is very similar, with a standard deviation of $0.25$ dex. Nevertheless, we find a systematic of-sett of $\log(M\un{*,ANN}) - \log(M\un{*,G}) = -0.11$ dex, which is caused by the different stellar mass estimation methods used on the two surveys. This is a problem inherent to the variety of possible stellar population synthesis models, based on different Initial Mass Functions (IMFs). In general, it has thus far been impossible to constrain stellar masses within a systematic uncertainty of $\sim 0.1$ dex (reference Angus ...) . In this work we therefore normalize the $M\un{*,ANN}$ values of our GL-KiDS sample to the mean $M\un{*,G}$ of GAMA, while indicating throughout our results the range of possible bias due to a $0.1$ dex shift in stellar mass. 
