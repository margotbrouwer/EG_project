...
After measuring the lensing profile around a galaxy sample, the next important step is to convert it into the corresponding RAR. We start from the ESD $\Delta\Sigma(R)$ as a function of projected radius and the measured stellar masses $M\un{*}$ of the galaxies, and aim to arrive at their total observed radial acceleration $g\un{obs}$ as a function of the expected baryonic radial acceleration $g\un{bar}$. To make sure the results are robust, we compare two different methods: a simple analytical method which assumes that DM haloes can be roughly approximated with a Singular Isothermal Sphere (SIS), and an elaborate numerical approach which fits a piece-wise power law to the stacked ESD profile (without any assumption on the averaged halo shape except for spherical symmetry). We validate both methods using mock surface density maps from the BAHAMAS simulation.

\subsection{Singular Isothermal Sphere approximation}
\label{SIS_approximation}



\subsection{Piece-wise power law density profile}
\label{piece-wise_powerlaw}
