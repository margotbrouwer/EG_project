...
% The HOD model MICE starts from the galaxy luminosity function and then converts it to stellar masses: for ‘-c01’ this conversion has been updated to obtain a better fit to the provided SMF at z= 0. For ‘-c02’ this has been implemented even more self-consistently for all redshifts while also changing the calibration of the star formation rate.
...
% We randomly select $100$ central subhaloes at $z=0.25$ from the `Dark Matter-only' simulation in each of a series of $0.25 \dex$ bins in $12 < \log_{10}(M_{500}/{\rm M}_\odot) < 15$ (in the highest mass bin there are less than 100 haloes; we select them all). In order to construct a consistent sample, the same haloes are then identified in the fiducial and hi/low AGN volumes.
...
% Is the SIS-like profile in MICE produced by sattelites? It seems unlikely, since the ESD profile of isolated galaxies with satellites f_M* < 10% of the lens stellar mass is equal to that with f_M* < 1% of the stellar mass!

\section*{Acknowledgements}
\textcolor{red}{To be adjusted. Kyle note: my funding acknowledgements revised and complete (11/09/2020).}

%\begin{comment}
KAO acknowledges support by the European Research Council (ERC) through Advanced Investigator grant to C. S. Frenk, DMIDAS (GA 786910), and support from VICI grant 016.130.338 of the Netherlands Foundation for Scientific Research (NWO).
