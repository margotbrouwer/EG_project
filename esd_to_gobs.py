...
def calc_sis(sigma_v, r, R):
    ...
    return gobs_sis, Mobs_sis, rho_sis, esd_sis
...
rmin, rmax = np.array([10., 5000.]) # in pc
...
gobs_sis, Mobs_sis, rho_sis, esd_sis = calc_sis(sigma_v, rcenters*xvalue, Rcenters*xvalue)
print('ESD(SIS):', esd_sis)

"""
# Plot the mock ESD profile

plt.plot(Rcenters, esd_sis, marker='o', ls='')
plt.xscale('log')
plt.yscale('log')
...
def calc_Cmn(R, r, m, n):
    ...
    if r[n+1] < R[m]: # If outer r < inner R: ))||
        C_mn = 0.
    elif (r[n+1] < R[m+1]): # If outer r < inner R: )|)|
        C_mn = -(4./3.) / (R[m+1]**2. - R[m]**2.) * \
        ( r[n+1]**3. - R[m]**3. - (r[n+1]**2. - R[m]**2.)**(3./2.) )
    elif r[n] < R[m]: # If inner r < inner R: )||)
        C_mn = -(4./3.) / (R[m+1]**2. - R[m]**2.) * \
        ( (r[n+1]**2. - R[m+1]**2.)**(3./2.) - (r[n+1]**2. - R[m]**2.)**(3./2.) \
        + R[m+1]**3. - R[m]**3. )
    elif r[n] < R[m+1]: # If inner r < outer R: |)|)
        C_mn = -(4./3.) / (R[m+1]**2. - R[m]**2.) * \
        ( (r[n+1]**2. - R[m+1]**2.)**(3./2.) - (r[n+1]**2. - R[m]**2.)**(3./2.) \
        - (r[n+1]**2. - R[m]**2.)**(3./2.) + R[m+1]**3. - r[n]**3. )
...
# Matrix Cmn that calculates the surface density Sigma(R)
...
# Matrix Bmn that calculates the average surface density avg(Sigma)(<R)
...
#test = np.array([[ a+b for a in np.arange(6) ] for b in np.arange(3)])
#print(np.sum(test, 0))
